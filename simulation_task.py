...
class SimulationTask(object):
    ...
    def __repr__(self, level=0):
        """
        :param level: Traverse the simulation tree and print out an overview of all simulations.
        :return:
        """
        # TODO: fix print format bug
    ...
    def parse_config_file(self):
        ...
        :return: return 0 if succeed, -1 if failed (file not exist, and cannot be created). If the file does not exsit
        but a new file with default values is created, the method returns 1.
        """
        conf_fn = os.path.join(self.fulldir, self.config_file)
        conf = cp.ConfigParser()
        if os.path.isfile(conf_fn):
            conf.read(conf_fn)
            self.config = conf
        else:
            print('WARNING: Simulation configuration file not exists! Creating default configuration as SiMon.conf.\n')
